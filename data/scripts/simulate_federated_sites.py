"""Ten federated MR sites sharing one quadratic causal curve.

Reuses simulate_nonlinear() from simulate_basic.py so every site follows the
same G -> X -> Y model (see docs/mr-simulation-model.md); only the per-site
...
theta1, theta2 (the quadratic causal curve) are fixed across all sites: the
causal effect of X on Y is assumed to be biology, not geography.
"""
...
from simulate_basic import simulate_nonlinear  # noqa: E402
...
def main():
    ...
    p.add_argument("--theta1", type=float, default=0.3, help="quadratic: slope at X=0, shared across sites")
    p.add_argument("--theta2", type=float, default=0.15, help="quadratic: curvature, shared across sites")
    ...
    p.add_argument("--out", type=Path, default=Path("simulated_data/federated"))
    a = p.parse_args()
    ...
    for i in range(a.n_sites):
        ...
        df, truth = simulate_nonlinear(
            int(n[i]), a.n_snps, "quadratic", a.theta1, a.theta2,
            float(h2_x[i]), float(gamma_x[i]), float(gamma_y[i]), site_seed,
        )
    ...
    (a.out / "manifest.json").write_text(json.dumps({
        "theta1": a.theta1, "theta2": a.theta2, "n_snps": a.n_snps, "seed": a.seed,
        "sites": manifest,
    }, indent=1))
