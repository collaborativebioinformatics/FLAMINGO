...
| `scripts/federated_exact_mr.py` | FedMR on every continuous set: exact federated 2SLS (classical and robust SE, first-stage F, cross-fitted variant) against the concatenated, site-meta and sumstats estimators; `results/fedmr.<shape>.csv` |
| `scripts/fedmr_sweep.py` | Seed sweep of FedMR, cross-fitted FedMR, site meta-analysis and sumstats IVW over sites, instrument strength, SNP count, imbalance, MAF shift, effect heterogeneity and pleiotropy |
| `tests/test_fedmr.py` | FedMR identity tests against stacked-data references and the pooled fits (`uv run pytest -q`) |
| `simulated_data/single/` | Single-site draws: `basic`, `basic_seed2`, `quadratic`, `threshold`, `cox` as `{csv,truth.json}` |
| `simulated_data/federated/<shape>/` | Ten-site federated draws, one per phenotype model plus `cox_rare` (small sites, short follow-up), `ushape` (θ1 = 0, zero average slope), `binary_quadratic_logistic` (0/1 outcome, no summary-statistics analysis yet) and `linear_shared` (the same 20 SNPs at every site, for the shared-instrument FedMR protocol): `site01..site10.{csv,truth.json}`, `manifest.{csv,json}`, and `sumstats/` from the analysis |
